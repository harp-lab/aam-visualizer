...
(define (regroup-by-call init states data-tables)
  ...
  #;(cons (get-li init)
          (foldl (lambda(c t)
                   (match-define (list nodes trans subs) t)
                   (match-define (list s-init s-nodes s-trans calls returns finals) (make-subgraph-follow c))
                   (define nodes+ (set-add nodes c))
                   (define trans+calls (hash-set trans c (for/hash ([c calls])
                                                           (values c (hash 'transition "call")))))
                   (define trans+returns (hash-set trans+calls c (for/hash ([r returns])
                                                                   (values r (hash 'transition "return")))))
                   (define trans+finals (hash-set trans+returns c (for/hash ([f finals])
                                                                   (values f (hash 'transition "stop")))))
                   (define subs+ (hash-set subs c (list s-init s-nodes s-trans)))
                   (list nodes+ trans+finals subs+))
                 (list (set)(hash)(hash))
                 (hash-keys all-calls)))
  (foldl (lambda(c t)
